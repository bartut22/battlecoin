...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {AMBIENT_ZONES,FIELD_RECT}=await import('/src/ambience-layout.js')
    ...
    const first=capture(0),second=capture(6.5),third=capture(12)
    ...
    const fieldChange=count(first,third,FIELD_RECT)
    ...
    return {differences,fieldChange,blockedDifference,reducedDifference,eventMode:ambience.scene.eventMode}
  })
